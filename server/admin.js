...
export default function Admin() {
  ...
  const handleLogin = (e) => {
    ...
    if (!socket) {
      setAuthError('Server connection offline. Please wait and retry.');
      return;
    }
    // The password is verified server-side (ADMIN_PASSWORD env var on the
    // backend) — nothing about what counts as valid ships in this bundle.
  ...
  useEffect(() => {
    ...
    // A reconnect (network blip, Render cold start) gives the server a
    // brand-new socket that hasn't passed admin_login yet. Silently
    // re-send the already-entered password so admin actions don't start
    // failing mid-event without any visible cause.
    const reauth = () => {
      ...
      {!isAuthenticated ? (
        ...
        <header className="flex flex-wrap items-center justify-between gap-4 p-5 rounded-2xl bg-[#302f27] border border-white/10 shadow-[0_4px_18px_-4px_rgba(0,0,0,0.25)]">
          ...
          <div className="flex items-center gap-3">
            ...
            {/* CSV Export */}
            <a
              href={`${import.meta.env.VITE_SERVER_URL || 'http://localhost:3001'}/api/export?token=${encodeURIComponent(password)}`}
              download
